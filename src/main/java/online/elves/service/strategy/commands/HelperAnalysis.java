...
public class HelperAnalysis extends CommandAnalysis {
    ...
    public void process(String commandKey, String commandDesc, String userName) {
        ...
        Fish.sendMsg("亲爱的 @" + userName + " " + CrLevel.getCrLvName(userName) + " " + " : 你好呀~\n" +
                ...
                "* `" + cmdSet + "  天降鱼丸` 和小冰亲密度大于`4096`的渔民大人, 每天可以自主开启一次天降鱼丸哦~ \n" +
                "* `" + cmdSet + "  爱的回馈` 和小冰的亲密度大于`10240`后, 每天可以获得[32,64]的一个随机爱的回馈红包哦~加油 \n" +
                "* `" + cmdSet + "  窝囊费` 每周六给你一个加班补贴, 慰藉你受伤的心灵(感谢老王(巫昂)发起活动, 这次不需要赞助!)~ \n" +
                "* `" + cmdSet + "  摸渔网` 消耗`5个鱼翅`, 去沙滩赶海...看看你能不能摸到渔网哦~ \n" +
                "* `" + cmdSet + "  捡鱼叉` 消耗`5个鱼翅`, 去沙滩赶海...看看你能不能捡到鱼叉哦~ \n" +
                "* `" + cmdSet + "  biu/标记  xxxx` 消耗`1个鱼叉`, 标记一个赶海的渔民(xxxx 渔民用户名! 不是昵称哦~), 扎破他的背包, 看看你能拿到什么吧~ \n" +
                "</details>" +
